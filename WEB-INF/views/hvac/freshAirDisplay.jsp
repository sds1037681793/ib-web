...
<body>
				...
				<tr>
					...
					<td align="right" width="170px">
						<button id="btn-query-parkingSpaceArea" type="button"
							class="btn btn-default btnicons" style="margin-left: 3rem;">
	...
	<div id="tb_freshAirDataDiv" class="content-default" style="min-height: 100px; width: 1700px;">
...
<script type="text/javascript">
	...
	var g_gatewayAddr;
	var ctx = '${ctx}';
	var code = "HEAT_FRESH_AIR_MACHINE";
	var locationType = 5;
	var oneCount = 30;
	var returnData;
	var firstReturnData;
	...
	$(document).ready(function() {
	 ...
	 function getFreshData(load_index) {
			//第一遍查询不需要再进行查询一次
			if (load_index == 1) {
				firstPageInit();
			} else {
	...
	function firstPageInit() {
		...
		for (var i = 0; i < dataLength; i++) {
			...
			html = '<li class="item">'
					+ '<div class="float-lf">'
					+ '<img class="item-img" src="static/img/device/FreshAir.png"/>'
					+ '</div>'
					+ '<div class="float-lf item-baseinfo">'
					+ '<div class="">'
					+ '<p class="item-equip" id=code_'+deviceId+'>'
					+ code
					+ '</p><p class="item-name">新风机</p>'
					+ '</div>'
	...
	function deviceInit() {//初始设备
		...
		$.ajax({
				...
				success : function(data) {
					deviceData = data;
					$("#tb_freshAirDataDiv").find("div").remove();
					...
					var nodataDiv= '<div style="width:100%;height:600px;padding-top: 3rem;font-size:20px;line-height:600px;text-align:center;">'
						+"无数据"+'</div>';
						$("#tb_freshAirDataDiv").append(nodataDiv);
