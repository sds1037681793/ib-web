...
<body>
				...
				<tr>
					...
					<td align="right" width="170px">
						<button id="btn-query-parkingSpaceArea" type="button"
							class="btn btn-default btnicons" style="margin-left: 2rem;">
	...
	<div id="tb_airCondiDataDiv" class="content-default" style="min-height: 100px; width: 1700px;">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#btn-query-parkingSpaceArea').on('click', function() {
			deviceInit();
		});
		toSubscribe();
	});
 ...
 function againPage(deviceData) {//再次请求页面
		$.ajax({
			type : "post",
			url : "${ctx}/hvac/freshAirDisplay/airConditioner",
	...
	function firstPageInit() {
		...
		for (var i = 0; i < dataLength; i++) {
			...
			//送风温度
			var supplyWindTemperature = "-    -";
			if (data[i].supplyWindTemperature) {
				supplyWindTemperature = data[i].supplyWindTemperature+"℃";
			}
			//回风温度
			var returnWindTemperature = "-    -";
			if (data[i].returnWindTemperature) {
				returnWindTemperature = data[i].returnWindTemperature+"℃";
			}
			//过滤网
			var alarmStatus = "-    -";
			...
			html = '<li class="item">'
					+ '<div class="float-lf">'
					+ '<img class="item-img" src="static/img/device/kongtiaojizu.png"/>'
					+ '</div>' + '<div class="float-lf item-baseinfo">'
					+ '<div class="">'
					+ '<p class="item-equip" id=code_'+deviceId+'>'
					+ code
					+ '</p><p class="item-name">空调机组</p>'
					+ '</div>'
					+ '<div class="equip-status clearfix">'
					+ '<p class="float-lf normal-width">开关：<span id=workStatus_'+deviceId+' >'
					+ workStatus
					+ '</span></p><p class="float-rt normal-width">故障状态：<span id=faultStatus_'+deviceId+' >'
					+ faultStatus
					+ '</span></p></div>'
					+ '<div class="equip-place clearfix">'
					+ '<p class="float-lf  position-device">位置：<span id=locationName_'+deviceId+'>'
					+ locationName
					+ '</span></p><p class="float-rt normal-width">手自动：<span id=automatic_'+deviceId+'>'
					+ automatic
					+ '</span></p></div>'
					+ '<div class="equip-status clearfix">'
					+ '<p class="float-lf normal-width">回风：<span id=returnWindTemperature_'+deviceId+'>'
					+ returnWindTemperature
					+ '</span></p><p class="float-rt normal-width">送风：<span id=supplyWindTemperature_'+deviceId+'>'
					+ supplyWindTemperature
					+ '</span></p></div>'
					+ '<div class="equip-place clearfix">'
					+ '<p class="float-lf normal-width">过滤网：<span id=alarmStatus_'+deviceId+' >'
					+ alarmStatus
					+ '</span></p></div>'
					+ '</div>'
					+ '</div>'
			...
			if(data[i].alarmStatus==1){
				$("#alarmStatus_" +deviceId).addClass("status-color");
		    }else if(data[i].alarmStatus==0){
		    	$("#alarmStatus_" +deviceId).addClass("statuss-color");
		    }
		}
	}
	...
	function deviceInit() {//初始设备
		...
		$.ajax({
				...
				success : function(data) {
					deviceData = data;
					$("#tb_airCondiDataDiv").find("div").remove();
					...
					var nodataDiv= '<div style="width:100%;height:600px;padding-top: 3rem;font-size:20px;line-height:600px;text-align:center;">'
						+"无数据"+'</div>';
						$("#tb_airCondiDataDiv").append(nodataDiv);
