	...
	<div class="water_bg">
		...
		<div class="detail_info" id="detailInfo" name="deviceInfo">
            ...
            <div class="lf">
            	...
            	<div id="pressure-info-div" style="width:338px;height: 295px;"></div>
            </div>
...
<script type="text/javascript">
	...
	function getEcharsData(id,divId,deviceId) {
			$.ajax({
				type : "post",
				url : "${ctx}/report/" + id +"?beginTime=" + beginTime+ "&endTime=" + endTime+ "&projectCode=" + projectCode+"&deviceId="+deviceId,
	...
	function getBaseDeviceInfo(){
		$.ajax({
			type: "post",
			url: "${ctx}/fire-fighting/fireFightingManage/getWaterDeviceBaseInfo?projectCode="+projectCode,
			async: false,
			contentType: "application/json;charset=utf-8",
			success: function(data) {
	...
	function getWaterDevicePosition(deviceNumber){
		$.ajax({
			type: "post",
			url: "${ctx}/fire-fighting/fireFightingManage/getDeviceByDeviceNumber?projectCode="+projectCode+"&deviceNumber="+deviceNumber,
			async: true,
			contentType: "application/json;charset=utf-8",
			success: function(data) {
				debugger;
	...
	function setValue(dto){
		...
		endTime = nowTimes();
		getEcharsData(138,"pressure-info-div",deviceId);
	...
	function toSubscribe(){
		if (isConnectedGateWay) {
			// 暖通空调首页信息识别结果
			stompClient.subscribe('/topic/ffmWaterSystemMapData/' + projectCode, function(result) {
	...
	function unloadAndRelease() {
		if(stompClient != null) {
			stompClient.unsubscribe('/topic/ffmWaterSystemMapData/' + projectCode);
		}
	}
	...
	function nowTimes(){
		...
		var now=year+'-'+ps(month)+"-"+ps(date)+" "+ps(h)+':'+ps(m)+":"+ps(s);
		return now;
	}
	...
	function getDeviceData(){
		  $.ajax({
		        type : "post",
		        url : "${ctx}/device/manage/getDeviceRelateData?projectId="+projectId+"&systemCode=FIRE_TERMINAL_WATER_TEST_SYSTEM",
		        dataType : "json",
		        async: false,
		        contentType : "application/json;charset=utf-8",
		        success : function(data) {
	...
	function buildBg(data){
		...
		$(".water_bg").css("background-image",'url('+data[0].filePath+')');
 		buildDiv(data);
	}
	
	function buildDiv(data){
		$.each(data,function(i,val){
			var div = '<div class="point" style="left: '+val.xCoordinate+'px; top:'+val.yCoordinate+'px;">'+
					  '<img id="'+val.deviceNumber+'"'+
					  'src="${ctx}/static/img/waterMap/xiaofangzhengchang.svg">'+
					  '<p id="p-'+val.deviceNumber+'" class="point-p-info" style="width: 83px;margin-left: -22px;"></p>'+
					  '</div>';
			$(".water_bg").append(div);
		})
	}
