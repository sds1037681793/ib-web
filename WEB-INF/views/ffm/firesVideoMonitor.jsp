...
<link href="${ctx}/static/styles/iconic.css" type="text/css"
	rel="stylesheet" />
<link
	href="${ctx}/static/component/jquery-datetimepicker/2.1.9/css/jquery.datetimepicker.css"
	type="text/css" rel="stylesheet" />
<link
	href="${ctx}/static/component/bootstrap-switch/3.3.2/css/bootstrap3/bootstrap-switch.min.css"
	type="text/css" rel="stylesheet" />
<link href="${ctx}/static/component/jquery-icheck/1.0.2/css/all.css"
	type="text/css" rel="stylesheet" />
<link href="${ctx}/static/styles/rib.css" type="text/css"
	rel="stylesheet" />
<link href="${ctx}/static/css/frame.css" type="text/css"
	rel="stylesheet" />
<link href="${ctx}/static/css/firesVideoForCloud.css" type="text/css"
	rel="stylesheet" />
<link href="${ctx}/static/css/cloudModleIframeBlue.css" type="text/css"
	rel="stylesheet" />
<link href="${ctx}/static/css/dropdownlistStyleForCloud.css"
	type="text/css" rel="stylesheet" />
...
<script src="${ctx}/static/component/wadda/commons.js"
	type="text/javascript"></script>
<script src="${ctx}/static/component/wadda/wadda.js"
	type="text/javascript"></script>
<script
	src="${ctx}/static/component/jquery-dropdownlist/jquery.dropdownlist.js"
	type="text/javascript"></script>
<script
	src="${ctx}/static/component/jquery-datetimepicker/2.1.9/js/jquery.datetimepicker.js"
	type="text/javascript"></script>
<script src="${ctx}/static/js/dropdownlistBule.js"
	type="text/javascript"></script>
...
<body>
	...
	<span id="operator-id" style="display: none"> <shiro:principal
			property="id" />
	</span>
	<span id="login-name" style="display: none"> <shiro:principal
			property="loginName" />
	</span>
	<div style="padding-top: 20px;">
		<table class="fires-title">
			<td class="alarm-describe">报警点</td>
			<td><img src="${ctx}/static/img/alarm/huojing.svg"
				id="firesAlarmTip" style="display: block;"></td>
			<td>
		...
		<div id="video-show" class="right_box">
			<iframe src="${ctx}/videomonitoring/showVideo?height=340&width=610"
				id="video-iframe" name="video-iframe" frameborder="0" scrolling="no"></iframe>
		</div>
		...
		<div class="result-describe" id="result-describe-div">
			...
			<div id="ffm-fires">
				...
				<div id="confirm-reason">
					...
					<div id="confirm-reason-checkbox"
						style="margin: 12px 10px 15px 37px;"></div>
				</div>
				<div id="mark-div">
					<textarea maxlength="20" id="confirm-reason-mark"
						placeholder="请描述火灾发生原因" onkeydown="checkMaxInput(this,20)"
						onkeyup="checkMaxInput(this,20)" onfocus="checkMaxInput(this,20)"
						onblur="checkMaxInput(this,20);"></textarea>
				</div>

				<!-- 消防确认 -->
				<div id="submitValue">
					<div id="alarm-record-confirm-div-button-confirm"
						class="btn-confirm-select" onclick="updateConfirm();">确定</div>
					<div class="btn-confirm-cancal" data-dismiss="modal"
						onclick="canal()">取消</div>
				</div>
			</div>
			<!-- 动态加载页面 -->
	...
	<script type="text/javascript">
		...
		function ffmView() {
			...
			var ffmTable = '<div id="ffm-view" class="ffm-view">' + '<div id ="event-confirm" class="event-confirm">' + eventConfirm + ' </div>' + '<div id ="event-confirm-type" class="event-confirm-type">火灾原因：' + confirmType + '</div>' + '<div id ="event-mark" class="event-mark">原因描述：' + mark + '</div>' + '</div>';
		...
		function initFiresVideoDropDownList() {
			...
			// 			firesVideoObj = $("#cameraName-dropdownlist").dropDownList({
			...
			$.ajax({
				...
				success : function(data) {
					...
					if (data.confirmResult == 1 || data.confirmResult == 2) {
						parent.showDialogModal("ffm-event-error-div", "提示", "该火警已被确认");
