...
<div id="reportDiv" style="display: none">
	...
	<div class="content-default" style="text-align: center;">
		<button id="saveEditReport" type="button" class="btn btn-default btn-common">保存</button>
		<button id="saveAs" type="button" class="btn btn-default btn-common">另存为</button>
	</div>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#saveEditReport').on('click', function(event) {
			saveEditReport();
		});
		...
		$.ajax({
			...
			success : function(data) {
				if (data && data.CODE && data.CODE == "SUCCESS") {
					...
					showDialogModal("error-div", "保存失败", data.MESSAGE, "", "", true, "");
					return;
				}
			},
	...
	function saveEditReport() {
		...
		$.ajax({
			type : "post",
			url : "${ctx}/report/update/" + reportId,
			dataType : "json",
			data : JSON.stringify(resultObject),
			contentType : "application/json;charset=utf-8",
			success : function(data) {
	...
	function editReport() {
		...
		$.ajax({
			type : "get",
			url : "${ctx}/report/getReportBean/" + reportId,
			dataType : "json",
			contentType : "application/json;charset=utf-8",
			success : function(data) {
				if (data != undefined) {
					analyseResult(data);
					document.getElementById("querySql").value = data.querySql;
					document.getElementById("reportName").value = data.reportName;
					document.getElementById("rowlimit").value = data.rowlimit;
					document.getElementById("exportFileName").value = data.exportFileName;
					document.getElementById("formCode").value = data.formCode;
					document.getElementById("height").value = data.height;
					document.getElementById("exportMaxNum").value = data.exportMaxNum;
				}
	...
	function previewReport() {
		...
		createModalWithLoad("perview-oper", window.innerWidth, 0, "预览报表", "report/previewReport", "closePerviewModal()", "confirm-close", "", "10");
		$("#perview-oper-modal").modal('show');
	}

	function closePerviewModal() {
		$("#perview-oper-modal").modal('hide');
	}
