(function($){
    jQuery.fn.dropDownListBule = function(options) {
        var defaults = {
            inputName: "Q",
            inputValName: "",
            buttonText: "示例",
        ...
        return this.each(function() {
            var opts = options;
            var ulwidth = parseInt(opts.width.replace("px",""))+60;
            var obj = $(this);
            var requiredStr = "";
            if (opts.required) {
            ...
            var dropDownListStr = "<div class='allBuleGroup' style='width:"+ulwidth+"px;'>";
            dropDownListStr = dropDownListStr + "<div class='input-group'>";
            ...
            dropDownListStr = dropDownListStr + "<input type='text' class='form-controls' name='" + opts.inputName + "' id='" + opts.inputName + "' style='width: " + opts.width + ";' " + requiredStr + " />";
            ...
            dropDownListStr = dropDownListStr + "<button  id='ddl-btn-" + opts.inputName + "' type='button'  class='btnbule dropdown-toggle' data-toggle='dropdown'>" + opts.buttonText + " <span class='caret'></span></button>";
            dropDownListStr = dropDownListStr + "<ul id='" + opts.inputName + "-ul' class='dropdown-menu dropdown-menublue-right' role='menu' style='width: "+ulwidth+"px;'>";
            dropDownListStr = dropDownListStr + "<li><ul id='" + opts.inputName + "-ul-li-ul' class='dropdown-menublue ' style='display:inherit;position:inherit;top:0;float:inherit;padding:0;border:0px;border-radius:0px;-webkit-box-shadow: inherit;box-shadow: inherit;'>";
            if(opts.search){
                dropDownListStr = dropDownListStr + "<input id='search' name='search' class='form-controls' type='text' style='border-bottom-right-radius: 3px;border-top-right-radius: 3px;margin-bottom: 10px;margin-left: 10px;margin-right: 20px;width:"+opts.width+";'  />";
            }
            ...
            dropDownListStr = dropDownListStr + "</ul></li></ul></div></div></div>";
