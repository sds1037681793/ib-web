(function($) {
	jQuery.fn.dynamicPreviewReport = function(options) {
		var defaults = {
			reportType : "3", // MMGrid
			colPerRow : "2"
		};
		...
		return this.each(function() {
			...
			$.ajax({
				...
				dataType : "json",
				data : JSON.stringify(opts.previewObject),
					...
					if (dynamicTableItems) {
						$.each(dynamicTableItems, function(n, item) {
							...
							if (object != undefined && object != null) {
								queryParams[item.inputName] = object.val();
							}
						});
					}
					queryParams["reportBean"] = JSON.stringify(opts.previewObject);
					return queryParams;
				},
				plugins : [
					pg
		...
		function exportExecl() {
			...
			$.each(dynamicTableItems, function(n, item) {
				...
				if (object != undefined && object != null) {
					prams = prams + "&" + item.inputName + "=" + $("#" + item.inputName).val();
				}
			});
