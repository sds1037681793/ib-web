		...
		var KEY0 = 48,
			...
			createDateTimePicker = function( input ) {
				...
				if (options.containerId) {
					$("#" + options.containerId).append(datetimepicker);
				} else {
				...
				var setPos = function() {
					...
					if (!options.containerId) {
						...
						var offset = datetimepicker.data('input').position();
						top = offset.top + datetimepicker.data('input').outerHeight() + datetimepicker.data('input').outerHeight(true) - datetimepicker.data('input').outerHeight() + 1;
						left = offset.left + datetimepicker.data('input').outerWidth(true) - datetimepicker.data('input').outerWidth();
						if (datetimepicker.data('input').offset().top + datetimepicker.outerHeight(true) > $(window).height() + $(window).scrollTop()) {
							top = offset.top - datetimepicker.outerHeight(true) - 1;
						}
						console.info("input的top位置：" + offset.top);
						console.info("input的height：" + datetimepicker.data('input').outerHeight());
						console.info("input的left位置：" + offset.left);
						console.info("最终位置top=" + top);
						console.info("最终位置left=" + left);
					}
					datetimepicker.css({
						position: position,
						left: left,
