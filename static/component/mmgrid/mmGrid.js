...
!function($){
    ...
    MMGrid.prototype = {
        ...
        _initLayout: function($el){
            ...
            if(opts.indexCol){
                opts.cols.unshift({title:'序号',width: '100', align: 'center' ,lockWidth: true, indexCol:true, renderer:function(val,item,rowIndex){
            ...
            if(opts.checkCol){
                ...
                opts.cols.unshift({title:chkHtml,width: 30, align: 'center' ,lockWidth: true, checkCol: true, renderer:function(){
        ...
        , _initHead: function(){
            ...
            if(opts.height !== 'auto'){
            	var headerHeight = $headWrapper.outerHeight(true);
            	if (headerHeight <= 0) {
            		headerHeight = 33;
            	}
                $bodyWrapper.height($mmGrid.height() - headerHeight);
            }
        ...
        , _initEvents: function(){
            ...
            $body.on('click','td',function(e){
                var $this = $(this);
                var event = jQuery.Event("cellSelect");
                ...
                var event2 = jQuery.Event("cellSelected");
                event2.target = e.target;
                that.$body.triggerHandler(event2, [$.data($this.parent()[0], 'item'), $this.parent().index(), $this.index()]);
            });

            $body.on('dblclick','td',function(e){
                var $this = $(this);
                var event = jQuery.Event("doubleClicked");
                event.target = e.target;
                that.$body.triggerHandler(event, [$.data($this.parent()[0], 'item'), $this.parent().index(), $this.index()]);
            });
            ...
            $body.on('click','tr > td .mmg-check',function(e){
                ...
                if(this.checked){
                    ...
                    var event = jQuery.Event("checkSelected");
                    event.target = e.target;
                    that.$body.triggerHandler(event, [$.data($this.parents('tr')[0], 'item'), $($this.parents('tr')[0]).index()]);
                }else{
                    that.deselect($($this.parents('tr')[0]).index());

                    var event = jQuery.Event("checkDeselected");
                    event.target = e.target;
                    that.$body.triggerHandler(event, [$.data($this.parents('tr')[0], 'item'), $($this.parents('tr')[0]).index()]);
                }
            });
        ...
        , _rowHtml: function(item, rowIndex){
            ...
            if($.isPlainObject(item)){
                ...
                for(var colIndex=0; colIndex < leafCols.length; colIndex++){
                    ...
                    trHtml.push('"');
                    if(opts.nowrap && col.name != "" && col.name != undefined){
                    	trHtml.push(' title="');
                    	if(col.tips){
                    		if(typeof(col.tips) == "function"){
                    			 trHtml.push(col.tips(item[col.name],item,rowIndex));
                    		}else{
                    			trHtml.push(col.tips);
                    		}
                    	}else if(col.renderer){
                    		var tipsContent  = col.renderer(item[col.name],item,rowIndex);
                    		if(!reg.test(tipsContent)){
        ...
        , _setColsWidth: function(){
            ...
            $body.width('auto');
            var styleText = [];
            var firtstVisibleColPaddingStyleAddes = false;
            for(var colIndex=0; colIndex<$ths.length; colIndex++){
                ...
                if(col.hidden){
                	...
                	if (!firtstVisibleColPaddingStyleAddes) {
                		var firstColPaddingLeft = opts.firstColPaddingLeft || 15;
                		styleText.push('padding-left: ' + firstColPaddingLeft + 'px !important;')
                		firtstVisibleColPaddingStyleAddes = true;
                	}
                }
                styleText.push(' }');
            }
        ...
        , _loadAjax: function(args){
            ...
            if(opts.remoteSort){
                ...
                if(sortName){
                    ...
                    // kefei：方便页面处理，单独传递sortName和sortType
                    params.sortName = sortName;
                    params.sortType = sortStatus;
                }
            }
        ...
        , findRow: function(args) {
            var that = this;
            that.deselect("all");
            if (args) {
                var rows = this.rows();
                var fromRow = args.fromRow || 0;
                $.each(rows, function(n, v) {
                    if (fromRow <= n) {
                    	var error = false;
                    	$.each(args, function(m, arg) {
                            var operator = arg.operator.toUpperCase();
                            var val = false;
                            if (operator == "LIKE") {
                                val = eval("v." + arg.col + ".indexOf('" + arg.val+"') >= 0 ? true : false");
                            } else if (operator == "=" || operator == "==") {
                                val = eval("(v." + arg.col + " == '" + arg.val + "') ? true : false");
                            } else if (operator == ">" || operator == ">=" || operator == "<" + operator == "<=") {
                                val = eval("v." + arg.col + " " + arg.operator + " '" + arg.val + "' ? true: false");
                            }
                            if (!val) {
                            	error = true;
                            	return false;
        ...
        , scrollToRow: function(row, times) {
        	// console.info("row = " + row + " ; times = " + times);
            if (times > 0 ) {
                var height = this[0].offsetHeight;
                if (height && height > 0) {
                    this.parent().scrollTop(this[0].offsetHeight * row / this.rows().length);
                } else {
                	var that = this;
                    setTimeout(function() {
                    	that.scrollToRow(row, times - 1);
                    }, 200);
                }
            }
        }
