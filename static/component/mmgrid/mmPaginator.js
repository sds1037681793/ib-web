!function($){
    ...
    MMPaginator.prototype = {
        _initLayout: function(){
            ...
            var pgHtmls;
            if(typeof(opts.jumpPg) =='undefined' || opts.jumpPg){
            	pgHtmls = [
                           '<div class="totalCountLabel"></div>',
                           '<ul class="pageList"></ul>',
                           '<div class="limit" style="display: none;"><select></select></div>',
                           '<div class="jump"><input type="text" id="pageNum'+this.get(0).id+'" style="width: 50px;margin-left: 0px;float:left;"  placeholder="页码" class="pageNum form-control required" /><span id="btnJump'+this.get(0).id+'" class="btn-jump"  style="margin-left: 5px;margin-top: 4px;" >跳转</span></div>',
                       ];
            } else {
            	pgHtmls = [
                           '<div class="totalCountLabel"></div>',
            ...
            if(typeof(opts.jumpPg) =='undefined' || opts.jumpPg){
            	this.$jumpPg = $el.find('.pageNum');
    			this.$jumpPg.on("keyup", function(event) {
                ...
                $('#btnJump'+this.get(0).id).on('click', function(){
                	if(this.parentElement.firstChild.value == ''){
                        $(this.parentElement.firstChild.id).val('');
                        if($("#error-div") && $("#error-div").length > 0){
            				showDialogModal("error-div", "输入有误", "输入页码不能为空");
                        } else{
                        	$("body").append('<div id="error-div"></div>');
            				showDialogModal("error-div", "输入有误", "输入页码不能为空");
                        }
                	...
                	if(parseInt(this.parentElement.firstChild.value) <= 0){
                		$('#'+this.parentElement.firstChild.id).val('');
                        if($("#error-div") && $("#error-div").length > 0){
            				showDialogModal("error-div", "输入有误", "输入页码不能为0");
                        } else{
                        	$("body").append('<div id="error-div"></div>');
            				showDialogModal("error-div", "输入有误", "输入页码不能为0");
                        }
                	...
                	var tmpTotalCount = $el.data('totalCount');
                	var tmpLimit = $el.find('.limit select').val();

                	if(typeof(tmpTotalCount) !='undefined' || typeof(tmpLimit) !='undefined'){
                		 var totalPage = tmpTotalCount % tmpLimit === 0 ? parseInt(tmpTotalCount/tmpLimit) : parseInt(tmpTotalCount/tmpLimit) + 1;
                         totalPage = totalPage ? totalPage : 0;
                         if(parseInt($('#'+this.parentElement.firstChild.id).val()) > totalPage){
                             $('#'+this.parentElement.firstChild.id).val('');
                             if($("#error-div") && $("#error-div").length > 0){
                 				showDialogModal("error-div", "输入有误", "输入页码不能大于最大页码");
                             } else{
                             	$("body").append('<div id="error-div"></div>');
                 				showDialogModal("error-div", "输入有误", "输入页码不能大于最大页码");
                             }
                    ...
                    $el.data('page', parseInt(this.parentElement.firstChild.value));
                    that.$mmGrid.load();
                    $('#'+this.parentElement.firstChild.id).val('');
                });
            }
        ...
        , init: function($grid){
            ...
            if($grid.opts.indexCol){
            	var indexCol;
            	if($grid.opts.checkCol){
            		indexCol = $grid.opts.cols[1];
            	} else {
                	indexCol = $grid.opts.cols[0];
            	}
