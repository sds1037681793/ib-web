...
function displayFireFighting(data) {
	$("#fireNumber").html(data.fires);
	$("#slfaNumber").html(data.slfaNumber);
	$("#soaNumber").html(data.soaNumber);
	$("#tpaNumber").html(data.tpaNumber);
	$("#slbaNumber").html(data.slbaNumber);
...
function openFireFightingPage(code) {
	if(code=="FFM_OUTSIDE_PAGE"){
		createPage(null, "户外消防系统", null, "/fireFightingManage/ffmOutsidePage");
